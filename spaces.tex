...
\end{table}
 ...
 In what follows we will consider also some of the other real parabolic pairs whose complexification is the parabolic pair $(\mfrak{g},\mfrak{p})$. We will use uniform notation $\epsilon^i$, $i=1,\ldots,p$ for the elements of the basis the nilradical and by $e_i$ we will denote the dual basis defined  by the Killing form. The elements $e_i$ then span the nilradical of the opposite parabolic subalgebra. %TODO bazove elementy?
...
\begin{table}[H]\label{tbl:strongly_og}\begin{center}
  \begin{tabular}{CCCCC}
  \mfrak{g}_0 & \beta& t & \xi_i & \mu_i \\\hline
...
\subsection[Explicit singular vectors for su(m, n)]{Explicit singular vectors for $\mfrak{su}(m, n)$}

Since Verma modules are in particular highest weight modules, any homomorphism between them is determined by the image of the generating vector. This image is called singular vector it is, up to duality, precisely the invariant differential operator we seek. In this section we apply the recently developed F-method to find (and classify) all singular vectors in Verma modules for $\mfrak{su}(m, n)$ which are induced from scalar representation. 
...
The standard Borel subalgebra $\mfrak{b}$ of $\mfrak{g}$ is defined through $\mfrak{b} = \mfrak{h} \oplus \mfrak{n}$, where the nilradical $\mfrak{n}$ of $\mfrak{b}$ is given by \eqref{eq:niradical borel}. The subset $\Sigma=\{\alpha_1,\dots,\alpha_{n-1},\alpha_{n+1},\dots,\alpha_{n+m-1}\}$ of $\Pi$ generates the root subsystem $\Delta_\Sigma$ in $\mfrak{h}^*$. We associate to $\Sigma$ the standard parabolic subalgebra $\mfrak{p}$ of $\mfrak{g}$ by $\mfrak{p}= \mfrak{l} \oplus \mfrak{u}$, where the reductive Levi subalgebra $\mfrak{l}$ of $\mfrak{p}$ is defined by \eqref{eq:Levi subalgebra} and the nilradical $\mfrak{u}$ together with the opposite nilradical $\widebar{\mfrak{u}}$ are given by \eqref{eq:nilradical parabolic}. The $\Sigma$-height $\htt_\Sigma(\alpha)$ of $\alpha \in \Delta$ is
\begin{align}
...
The character $\rho \in \Hom_P(\mfrak{p},\C)$ defined by \eqref{eq:rho_vector} is
\begin{align}
...
\remark Let us note that the action of the Levi subalgebra can be written more succinctly using the following notation.
\medskip

\theorem{\label{thm:action AGS}
Let $\lambda \in \Hom_P(\mfrak{p},\C)$ and let $(\sigma,\mathbb{V})$ be a $\mfrak{p}$-module. Then the embedding of $\mfrak{g}$ into $\eus{A}^\mfrak{g}_{\widebar{\mfrak{u}}} \otimes_\C \End \mathbb{V}_{\lambda+\rho}$ and $\eus{A}^\mfrak{g}_{\widebar{\mfrak{u}}^*} \otimes_\C \End \mathbb{V}_{\lambda-\rho}$ is given by
\begin{enumerate}
\item[1)]
\begin{align}
\begin{aligned}
...
\proof{It follows immediately from Theorem \ref{thm:action 1-graded} using the commutation relations for the Lie algebra $\mfrak{sl}(n+m,\C)$.}
\vspace{-2mm}
...
%\begin{align}
%  \begin{aligned}
%    \pi_{\sigma_\lambda}(h_c)&= \bigg({1 \over n} + {1 \over m}\bigg) \sum_{k=1}^m \sum_{\ell=1}^n x_{k\ell} \partial_{x_{k\ell}} + \bigg(\lambda+ {n+m \over 2}\bigg) + \sigma(h_c), \\
%    \pi_{\sigma_\lambda}(h_{A,B})&= \sum_{k=1}^m \sum_{\ell=1}^n \sum_{r=1}^n a_{r\ell} x_{kr} \partial_{x_{k\ell}} - \sum_{k=1}^m \sum_{\ell=1}^n \sum_{s=1}^m b_{ks} x_{s\ell} \partial_{x_{k\ell}} + \sigma(h_{A,B}), \\
%    \hat{\pi}_{\sigma_\lambda}(h_c)&= -\bigg({1 \over n} + {1 \over m}\bigg) \sum_{k=1}^m \sum_{\ell=1}^n y_{k\ell} \partial_{y_{k\ell}} +  \bigg(\lambda - {n+m \over 2}\bigg) + \sigma(h_c), \\
%    \hat{\pi}_{\sigma_\lambda}(h_{A,B})&= -\sum_{k=1}^m \sum_{\ell=1}^n \sum_{r=1}^n a_{r\ell} y_{k\ell} \partial_{y_{kr}} + \sum_{k=1}^m \sum_{\ell=1}^n \sum_{s=1}^m b_{ks} y_{k\ell}\partial_{y_{s\ell}} + \sigma(h_{A,B})
%  \end{aligned}
%\end{align}
%for all $A=(a_{r\ell}) \in M_{n,n}(\C)$, $B=(b_{ks}) \in M_{m,m}(\C)$ satisfying $\tr A =0$ and $\tr B = 0$;
%\item[3)]
%\begin{align}
%  \begin{aligned}
%    \pi_{\sigma_\lambda}(e_{ij})&= \sum_{k=1}^m \sum_{\ell=1}^n x_{ki} x_{j\ell} \partial_{x_{k\ell}}  + \bigg(\lambda+ {n+m \over 2}\bigg) x_{ji} + \sigma(e_{ij}) + x_{ji}\sigma(h_c)
%    \\ &\quad + \sum_{\ell=1}^n x_{j\ell} \sigma(h_{E_{i\ell}-{1\over n}I_n \delta_{i \ell},0})
%     - \sum_{k=1}^m x_{ki} \sigma(h_{0,E_{kj}-{1 \over m}I_m \delta_{kj}}),  \\
%    \hat{\pi}_{\sigma_\lambda}(e_{ij})&= \sum_{k=1}^m \sum_{\ell=1}^n y_{k\ell} \partial_{y_{ki}} \partial_{y_{j\ell}} - \bigg(\lambda-  {n+m \over 2}\bigg) \partial_{y_{ji}} + \sigma(e_{ij}) - \partial_{y_{ji}} \sigma(h_c)
%    \\ &\quad - \sum_{\ell=1}^n \partial_{y_{j\ell}} \sigma(h_{E_{i\ell}-{1\over n}I_n \delta_{i \ell},0})
%     + \sum_{k=1}^m \partial_{y_{ki}} \sigma(h_{0,E_{kj}-{1 \over m}I_m \delta_{kj}})
%  \end{aligned}
%\end{align}
...
\subsection{Singular vectors and equivariant differential operators}

Let $\alpha \in \C$ and let $(\sigma,\C)$ be the trivial $\mfrak{p}$-module. Then from \eqref{eq:Phi isomorphism} we have an isomorphism
\begin{align}
  \tau \circ \Phi_{\lambda_\alpha} \colon M^\mfrak{g}_\mfrak{p}(\C_{\lambda_\alpha-\rho}) \riso \C[\widebar{\mfrak{u}}^*] \otimes_\C \C_{\lambda_\alpha-\rho}
\end{align}
...
%Moreover, because $\C[\widebar{\mfrak{u}}^*] \otimes_\C \C_{\lambda_\alpha-\rho}$ and also $\Sol(\mfrak{g},\mfrak{p};\C[\widebar{\mfrak{u}}^*] \otimes_\C \C_{\lambda_\alpha-\rho})^\mcal{F} \subset \C[\widebar{\mfrak{u}}^*] \otimes_\C \C_{\lambda_\alpha-\rho}$ are semisimple $\mfrak{l}$-modules, we denote by $(\C[\widebar{\mfrak{u}}^*] \otimes_\C \C_{\lambda-\rho})_\mu$ and $\Sol(\mfrak{g},\mfrak{p};\C[\widebar{\mfrak{u}}^*] \otimes_\C \C_{\lambda-\rho})^\mcal{F}_\mu$ the $\mfrak{l}$-isotypic components of highest weight $\mu \in \mfrak{h}^*$.
\medskip

\proposition{\label{prop:C[u] decomposition AGS}
...
\lemma{\label{lem:solution space AGS}
Let $\alpha \in \C$ and $\mu \in \Lambda^+_\alpha(\mfrak{p})$. Then we have
\begin{align*}
 \Sol(\mfrak{g},\mfrak{p};\C[\widebar{\mfrak{u}}^*] \otimes_\C \C_{\lambda_\alpha-\rho})^\mcal{F}_\mu \simeq
 \begin{cases}
...
\proof{Using Proposition \ref{prop:C[u] decomposition AGS} we know that $\C[\widebar{\mfrak{u}}^*] \otimes_\C \C_{\lambda_\alpha-\rho}$ is a multiplicity free $\mfrak{l}$-module. Moreover, since we have $\Sol(\mfrak{g},\mfrak{p};\C[\widebar{\mfrak{u}}^*] \otimes_\C \C_{\lambda_\alpha-\rho})^\mcal{F}_\mu \subset (\C[\widebar{\mfrak{u}}^*] \otimes_\C \C_{\lambda_\alpha-\rho})_\mu \simeq \mathbb{F}_\mu$ for $\mu \in \Lambda^+_\alpha(\mfrak{p})$, we obtain that $\smash{\Sol(\mfrak{g},\mfrak{p};\C[\widebar{\mfrak{u}}^*] \otimes_\C \C_{\lambda_\alpha-\rho})^\mcal{F}_\mu}$ is either the zero $\mfrak{l}$-module or isomorphic to the $\mfrak{l}$-module $\mathbb{F}_\mu$. Therefore, it is enough to verify, whether the highest weight vector $v_\mu$ of $(\C[\widebar{\mfrak{u}}^*] \otimes_\C \C_{\lambda_\alpha-\rho})_\mu$ is contained in $\Sol(\mfrak{g},\mfrak{p};\C[\widebar{\mfrak{u}}^*] \otimes_\C \C_{\lambda_\alpha-\rho})^\mcal{F}$ or not. Let $\mu \in \Lambda^+_\alpha(\mfrak{p})$ and let us assume that the highest weight vector
\begin{align*}
  v_\mu = q_1^{a_1}q_2^{a_2}\dots q_r^{a_r}
\end{align*}
of $(\C[\widebar{\mfrak{u}}^*] \otimes_\C \C_{\lambda_\alpha-\rho})_\mu$ with $a_1,a_2,\dots a_r \in \N_0$ belongs to $\Sol(\mfrak{g},\mfrak{p};\C[\widebar{\mfrak{u}}^*] \otimes_\C \C_{\lambda_\alpha-\rho})^\mcal{F}$. Because of the greater clarity, we denote by $q^a$ the polynomial $q_1^{a_1}q_2^{a_2}\dots q_r^{a_r}$ for $a=(a_1,a_2,\dots,a_r) \in \N_0^r$ and by $1_k \in \N_0^r$ the $r$-tuple having $1$ at the $k$-th position and $0$ otherwise. Furthermore, we have
\begin{align*}
...
If $v_\mu =1$, then $\hat{\pi}_{\lambda_\alpha}\!(e_{ij})v_\mu=0$ for all $i=1,2,\dots,n$, $j=1,2,\dots,m$ and $\alpha \in \C$. Therefore, we have $\Sol(\mfrak{g},\mfrak{p};\C[\widebar{\mfrak{u}}^*] \otimes_\C \C_{\lambda_\alpha-\rho})^\mcal{F}_\mu \simeq \mathbb{F}_\mu$ for $\mu=\big(\alpha-{n+m \over 2}\big)\omega_n$ and $\alpha \in \C$.
\smallskip
...
\begin{align*}
  ...
  &=  \sum_{k=s+1}^m  a_s  q_s^{a_s-1} \partial_{y_{ji}}q_s -(m-s)\partial_{y_{ji}}q_s^{a_s}=(m-s)\partial_{y_{ji}}q_s^{a_s}-(m-s)\partial_{y_{ji}}q_s^{a_s}=0,
\end{align*}
where we used $L^y_{kj}q_s=0$ if $j>s$ or if $k \leq s$ and $k\neq j$ in the third equality and $\partial_{y_{ki}}L^y_{kj}q_s=\partial_{y_{ji}} q_s$ for $k > s$ in the fifth equality. Hence, we have \smash{$\Sol(\mfrak{g},\mfrak{p};\C[\widebar{\mfrak{u}}^*] \otimes_\C \C_{\lambda_\alpha-\rho})^\mcal{F}_\mu \simeq \mathbb{F}_\mu$} for $\mu=a \omega_{n-k} + a\omega_{n+k}-(a+k)\omega_n$ and $\alpha = a-k + {n+m \over 2}$.}

\theorem{\label{thm:singular vectors decomposition AGS}
We have
\begin{align*}
  \tau \circ \Phi_{\lambda_\alpha+\rho} \colon M^\mfrak{g}_\mfrak{p}(\C_{\lambda_\alpha})^\mfrak{u} \riso
  \begin{cases}
...
\proof{The decomposition of the space of singular vectors $M^\mfrak{g}_\mfrak{p}(\mathbb{C}_{\lambda_\alpha})^\mfrak{u}$ is a straightforward consequence of Lemma \ref{lem:solution space AGS}.}
\vspace{-4mm}
...
\begin{align}
  \varphi \colon M^\mfrak{g}_\mfrak{p}(\mathbb{F}_{\mu_{\alpha,k}}) \rarr M^\mfrak{g}_\mfrak{p}(\C_{\lambda_\alpha})
\end{align}
...
\begin{align}
  \varphi_0 \colon \mathbb{F}_{\mu_{\alpha,k}} \rarr M^\mfrak{g}_\mfrak{p}(\C_{\lambda_\alpha})
\end{align}
of $\mfrak{p}$-modules, which is defined by
\begin{align}
  \varphi_0(v_{\mu_{\alpha,k}})= q^{\alpha+r-k}_{r-k}(f_{ij}) v_\alpha,
\end{align}
where $v_\alpha \in M^\mfrak{g}_\mfrak{p}(\C_{\lambda_\alpha})$ is the highest weight vector of $M^\mfrak{g}_\mfrak{p}(\C_{\lambda_\alpha})$ and $v_{\mu_{\alpha,k}} \in \mathbb{F}_{\mu_{\alpha,k}}$ is the highest weight vector of $\mathbb{F}_{\mu_{\alpha,k}}$.


The same approach works also in the $\mfrak{sp}$ and $\mfrak{so}^*$ cases \cite{krizka_invariant_2017}. 