...
We start with some algebraic preliminaries and then move on to the invariant differential operators.
...
For special case when $\lie{g}$ is a semisimple complex Lie algebra and $\lie{h} = \lie{b}$ is  its Borel subalgebra these induced modules $\lie{U(g)_{U(b)}}\otimes \mathbb{V}^*$ are known as Verma modules and for a irreducible $\lie{b}$ module $\mathbb{C}_\lambda$, $\lambda \in \mathbb{C}$ are usually denoted by $M(\lambda)$. If one takes for $\lie{h} = \lie{p}$ a parabolic subalgebra of $\lie{g}$, then one gets so called generalized or parabolic Verma modules $M_\lie{p} (\mathbb{F}_\lambda)$ where now $\mathbb{F}_\lambda$ denotes the irreducible representation of the Levi part $\lie{l}$ of $\lie{p}$. In this case, one has to of course restrict to $\lambda$ that is $\lie{l}$-integral and $\lie{l}$-dominant. 
...
The authors of \cite{huang_dirac_2006} proved that if $\rep{V}$ is a unitarizable $(\lie{g},K)$-module and $\lie{u}_+$ is abelian nilradical of a $\theta$-stable parabolic subalgebra, then the Hodge decomposition of $C_\bullet(\lie{u}_+,\rep{V})$ is still valid. 
...
\begin{proposition}[Proposition 6.6 of \cite{davidson_differential_1991}]
 ...
 Each set $\Lambda_a$ is a translated integral cone with vertex $\lambda_a + C_a$.
\end{proposition}
...
In this case we generate the whole original root system and thus the cohomology is in this case computed by the orginial Kostant's formula.   
