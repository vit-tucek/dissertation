...
These cohomologies are explicitely computed for the two expceptional types in \cite{enright_resolutions_2004-1}. The papers \cite{enright_hilbert_2004}, \cite{enright_resolutions_2004} treat also certain weights for the classical types $\lie{su}(p,q)$, $\lie{sp}(n,\R)$ and $\lie{so}^*(2n).$ So what remains to be done are the orthogonal cases $\lie{so}(2,2p)$ which are rather easy and are calculated completely in sections \ref{sec:conf_even} and \ref{sec:conf_odd}. For the other classical types we calculate some of the data that go into the formula \eqref{eq:cohomology} and calculate the cohomologies completely in small ranks in appendix \ref{app:cohomology}. We borrow idea from \cite{enright_resolutions_2004-1} and calculate possible $\Psi_\lambda^+ $ by labeling the poset of noncompact roots by scalar products of the associated coroots with $\lambda + \rho + \mu$ for $\mu \in C$. We show only nodes / roots where the scalar product is not always negative. 
...
\begin{example}
To illustrate the situation, here is an example for $\mathrm{SO}^*(16)$ and cone of unitarizable weights $\left(a_{5} + 1\right)\omega_{5} - \left(2 \, a_{5} + 2 \, a_{6} + a_{7} + 8\right)\omega_{8}.$
